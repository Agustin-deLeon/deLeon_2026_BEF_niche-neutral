...
#
# Specifically, it:
#   (1) imports and preprocesses species occurrence,
#       biomass, trait, and environmental datasets;
#   (2) computes Gower distances among species and performs
#       a PCoA of functional trait space;
#   (3) fits annual CATS models to partition the variation
#       in species occurrences explained by functional traits
#       while accounting for metacommunity abundances;
#   (4) calculates annual expected (R2.exp) and unexplained
#       (R2.unexp) trait effects, representing each year's
#       position along the niche–neutral gradient; and
#   (5) generates the data frames required for subsequent
#       BEF, functional diversity, NST, and figure analyses.
#
# Required input files:
#   - br.xlsx: species occurrence matrix.
#   - bm.xlsx: biomass matrix.
#   - Traits.xlsx / traits_numerico_n.csv: species trait data.
#   - Environmental.xlsx: environmental variables by pond.
#   - Ambientales_resumen.xlsx: annual environmental summaries.
#
# Main output objects:
#   - nn.1 (CATS annual estimates; includes R2.exp and
#     R2.unexp).
#   - bm.nuevo, bm.act: processed biomass datasets.
#   - br.muestreos: processed community matrix.
#   - profundidad: annual mean depth metrics.
#
# The object nn.1 is required by:
#   - BEF.R
