...
col_assembly <- adjustcolor("#006A19", alpha.f = 1)
col_bef      <- adjustcolor("#013744",  alpha.f = 1)
...
p1.B <-
  ...
  geom_point(
    ...
    fill = pt_A,       # relleno
    colour = "black",  # borde
    stroke = 1,        # grosor del borde
    size = 4
  ) +
  labs(
    x = "Interannual change in pre-sampling \n precipitation (Apr-Sep)",
    y = expression(
      atop(
        "Variance in species occurrences",
        paste(
...
plot(riqueza ~ R2.exp, data = nn.1,
     ...
     col.axis = ax_col,
     col.lab = ax_col,
     cex.lab = 2, cex.axis=1.5)   # tamaño de títulos de los ejes
...
par(mar = c(9, 9, 3, 3), mgp = c(3.5, 1, 0), cex.lab = 2.5, cex.axis = 1.8)
#col_bef  <- "#3A8EC1"
#col_biomasa <- "#E69F00"
col_biomasa <- "#AD6300"
col_bef  <- "#2F627F"
...
plot(biomasa.neutral ~ datos$R2.exp,
     ...
     bty = "l", pch = 21,
     bg = pt_biomasa,   # relleno
     col = "black",     # círculo exterior
     lwd = 1,  
     cex = 2.5,
...
plot(BEF.precipitation ~ datos$lluvia,
     xlab = " ", ylab = "",
     bty = "l", pch = 21, bg = pt_bef,   # relleno
     col = "black",     # círculo exterior
     lwd = 1,   cex = 2.5,
     col.axis = ax_col, col.lab = ax_col)
mtext("Annual precipitation", side = 1, line = 3.75, cex = 2, col = ax_col)
