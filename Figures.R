...
df_func <- data.frame(
  ...
  R2.exp = R2.exp,
  riqueza = riqueza,
  FRic = Modelo_nulo$FRic,
  FEve = Modelo_nulo$FEve,
  FDis = Modelo_nulo$FDis,
  FDiv = Modelo_nulo$FDiv,
  RaoQ = Modelo_nulo$RaoQ
)

df_func <- df_func[order(df_func$año), ]

stopifnot(
  all(df_func$año == Modelo_nulo$año),
  nrow(df_func) == length(R2.exp),
  nrow(df_func) == length(riqueza)
)


plot_panel_fd <- function(
    data,
    response,
    ylab,
    panel_letter,
  ...
  #-----------------------------------------
  # Posición automática de estadísticas
  #-----------------------------------------
  
  if (is.null(stats_x)) {
    stats_x <- min(x) + 0.68 * diff(range(x))
  }
  
  if (is.null(stats_y)) {
    stats_y <- min(y) + 0.90 * diff(range(y))
  }
  ...
  # Redibujar puntos por encima de la banda
  points(
    x,
    y,
    pch = 19,
  ...
  f_value <- unname(sm$fstatistic["value"])
  ...
  r2_value <- sm$r.squared
  
  p_txt <- if (p_value < 0.001) {
    "< 0.001"
  } else {
...
graficar_vias_h <- function(resultado, nombre_metrica){
  ...
  rect(0.5,2,2.8,3,lwd=2)
  rect(4.6,2,7.2,3,lwd=2)
  rect(9.0,2,11.5,3,lwd=2)
  
  text(1.65,2.5,expression(R[Traits]^2),cex=1.2)
  text(5.9,2.5,"Species richness",cex=1)
  text(10.25,2.5,nombre_metrica,cex=1.2)
  ...
  text(
    ...
    1.05,
    paste0(
      "c' = ",round(tab$efecto_directo_R2,2),
      "\np = ",signif(tab$p_directo,2)
    ),
    cex=.9
  )
  
  title(
    main=paste("Path analysis:",nombre_metrica),
    line=.4
  )
}
...
for (m in metricas_sem) {
  
  graficar_vias_h(
    resultado = resultados_sem[[m]],
    nombre_metrica = nombres_metricas[m]
  )
...
###############
# --- BEF 
datos$BEF <- pendientes.lineales[,2] # Here you can choose the BEF estimate you want to use for the analyses
bestglm(
  Xy = datos.z[-c(20,21), c(
    "R2.exp",
    "prof_media",
    "prof_CV",
    "lluvia",
...
# Here only are presented Fig.S2 and Fig.S3. The rest can be found at the following scripts:

# Fig.S1: "NST.R" -> It contains both NST analyses and the relation between NST and R2.traits.

# Fig.S4-S13: "BEF.R"-> It contains the necessary script for both estimate and visualize BEF slopes 
