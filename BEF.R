...
for (ii in 2005:2025) {
  ...
  plot(
    ...
    col = colors[as.character(df.um.ch$ch.id)],
    pch = 19,
    xlab = "Richness",
  ...
  # ---- Líneas punteadas por charco (solo en su rango de riqueza) ----
  for (i in seq_along(unique_ids)) {
    
    id_i <- unique_ids[i]
    current_data <- df.um.ch[df.um.ch$ch.id == id_i, ]
    ...
    # dibujar línea punteada solo en el rango del charco
...
par(mfrow=c(6,4), mar=c(5.6,6,3,2), cex.lab = 2, pty = "m")
...
# =========================================================
# CON COVARIABLES
# MISMO ESTILO VISUAL QUE LOS GLMM
# =========================================================
...
# slopes_df$P_quad <- ifelse(slopes_df$best_label=="Quadratic",1,0)
# 
# summary(glm(slopes_df$P_quad~nn.1$R2.unexp))
# summary(lm(slopes_df$deriv_S1~nn.1$R2.exp+nn.1$lluvia_anual))
# summary(lm(slopes_df$deriv_S2~nn.1$R2.exp+nn.1$lluvia_anual))
# 
# 
# 
# 
...
# slopes_df_sat$P_quad <- ifelse(slopes_df_sat$best_label=="Quadratic",1,0)
# 
# summary(glm(slopes_df_sat$P_quad~nn.1$R2.unexp))
# summary(lm(slopes_df_sat$deriv_S1~nn.1$R2.exp+nn.1$lluvia_anual))
# summary(lm(slopes_df_sat$deriv_S2~nn.1$R2.exp+nn.1$lluvia_anual))
...
double_curve(
  ...
  col  = col_bef)
#mtext("Varianza en ocurrencias de especies\nexplained by traits",
...
plot_glm_visreg_R2 <- function(modelo, xvar = "R2.exp",
                               xlab = #"Varianza en ocurrencias de especies\nexplained by traits",
                                 expression(
                                   atop(
                                     "Variance in species occurrences",
                                     paste("explained by traits (", R[plain(Traits)]^2, ")")
...
# )
# # mtext("Varianza en ocurrencias de especies\nexplained by traits",
# #       side = 1, line = 5.75, cex = 1.5, col = ax_col)
# mtext(expression(
#   atop(
#     "Variance in species occurrences",
#     paste("explained by traits (", R[plain(Traits)]^2, ")")
#   )), side = 1, line = 7, cex = 1.5, col = ax_col)
# text(x = 0.4, y = 0.8, "F(2,16): 3.9\nP: 0.04\nR²: 0.25", cex = 1.8,
#      pos = 3, col = ax_col)
# mtext("B", side = 3, line = 1.0, adj = 0, font = 2, cex = 2.0, col = ax_col)
# BEF.precipitation <- with(nn.1,
#                           BEF - p["R2.exp"] * R2.exp +
...
#     paste("explained by traits (", R[plain(Traits)]^2, ")")
#   )), side = 1, line = 7, cex = 1.5, col = ax_col)
# text(x = 0.4, y = 1, "F(2,16): 3.1\np: 0.07\nR²: 0.2", cex = 2,
#      pos = 3, col = ax_col)
# mtext("C", side = 3, line = 1.0, adj = 0, font = 2, cex = 1.5, col = ax_col)
# x.seq <- seq(
#   min(nn.1$R2.exp, na.rm = TRUE),
#   max(nn.1$R2.exp, na.rm = TRUE),
#   length.out = 200)
...
#   c(lwr, rev(upr)),
#   border = NA,
#   col = adjustcolor(col_D, alpha.f = 0.15))
# lines(x.seq, fit, col = col_D, lwd = 3)
# lines(x.seq, lwr, col = col_D, lty = 2, lwd = 1.5)
# lines(x.seq, upr, col = col_D, lty = 2, lwd = 1.5)
# 
# 
# BEF.precipitation <- with(nn.1,
#                           BEF - p["R2.exp"] * R2.exp +
